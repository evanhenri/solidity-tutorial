...
function App() {
  ...
  async function updateBallots() {
    ...
    for(let i = 0; i < nextBallotId; i++) {
      const [ballot, choices, hasVoted] = await Promise.all([
        contract.methods.ballots(i).call(),
        contract.methods.choices(i).call(),
  ...
  async function createBallot(e) {
    e.preventDefault();
    const name = web3.utils.asciiToHex(e.target.elements[0].value);
    const choices = e.target.elements[1].value
        .split(',')
        .map(s => web3.utils.asciiToHex(s));
          ...
          <table className="table">
            ...
            <tbody>
              ...
              {ballots.map(ballot => (
                <tr key={ballot.id}>
                  <td>{ballot.id}</td>
                  <td>{web3.utils.hexToUtf8(ballot.name)}</td>
                  <td>
                    <ul>
                    {ballot.choices.map(choice => (
                      <li key={choice.id}>
                        id: {choice.id},
                        name: {web3.utils.hexToUtf8(choice.name)},
                        votes: {choice.votes}
                      </li>
                    ))}
                    </ul>
                          ...
                          <select className="form-control" id="choice">
                            {ballot.choices.map(choice => (
                              <option
                                key={choice.id}
                                value={choice.id}>
                                {web3.utils.hexToUtf8(choice.name)}
                              </option>
                            ))}
                          </select>
                        </div>
